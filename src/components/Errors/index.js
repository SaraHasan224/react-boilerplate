...
function Error() {
  ...
  // const { message, status, description } = createMuiTheme state => state.error);
  ...
  //   const title = locale === CONSTANTS.SUPPORTED_LANG.URDU ? "Something went wrong" : ERROR_MSGS.EN.E_BAD_REQUEST
  ...
  return (
      <section className="errorPage">
        ...
        <div className="errorPageText">
          <h1>{locale === CONSTANTS.SUPPORTED_LANG.URDU ? ERROR_MSGS.UR.ERROR_SUBTITLE : ERROR_MSGS.EN.ERROR_SUBTITLE}</h1>
          <p>
            {status}
          ...
          <span onClick={redirectToRoot} className="btn btn-error-back">
            {locale === CONSTANTS.SUPPORTED_LANG.URDU ? ERROR_MSGS.UR.BACK_BTN_TEXT : ERROR_MSGS.EN.BACK_BTN_TEXT}
          </span>
        </div> */}
      </section>
  );
